...
{
    public class AgencyOwnerProjectInvoiceDetailsOutput : AgencyOwnerProjectInvoiceOutput
    {
        ...
        {
            get { return IndividualPayoutIntents.Count(x => !x.IsComplete) + OrganizationPayoutIntents.Count(x=>!x.IsComplete); }
        }

        public int PayoutsWithoutFinancialAccounts
        {
            get { return IndividualPayoutIntents.Count(x => !x.PayoutsEnabled) + OrganizationPayoutIntents.Count(x => !x.PayoutsEnabled); }
        }
