...
namespace AgencyPro.Common.EventHandling
{
    ...
    {
        private readonly ConcurrentDictionary<Type, Action<TTargetBase, TParamBase>> _actionCache =
            new ConcurrentDictionary<Type, Action<TTargetBase, TParamBase>>();
        ...
        public Action<TTargetBase, TParamBase> GetAction(TParamBase paramInstance)
        {
            ...
            if (!_actionCache.ContainsKey(paramType)) _actionCache[paramType] = BuildActionForMethod(paramType);
